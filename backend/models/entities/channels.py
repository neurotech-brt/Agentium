...
class ExternalChannel(BaseEntity):
    ...
    # Basic info
    name = Column(String(100), nullable=False)  # "Support WhatsApp", "Sales Slack"
    ...
    require_approval = Column(Boolean, default=False)  # Head approval for sensitive channels
    
    # Webhook URL (generated by backend for external services to call)
    webhook_path = Column(String(100), unique=True, nullable=True)  # /webhooks/whatsapp/abc123
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        base.update({
            ...
            'config': {
                ...
                'webhook_url': self.config.get('webhook_url_display')  # Partial for UI
            },
            'routing': {
...
class ExternalMessage(BaseEntity):
    ...
    sender_id = Column(String(200), nullable=False)  # Phone number, Slack user ID, email
    sender_name = Column(String(100), nullable=True)
    sender_metadata = Column(JSON, default=dict)  # Profile pic, email headers, etc.
    
    # Content
    message_type = Column(String(20), default="text")  # text, image, file, voice
    content = Column(Text, nullable=False)
    media_url = Column(String(500), nullable=True)  # For images/files
    raw_payload = Column(JSON, nullable=True)  # Original webhook payload
    
    # Processing
    status = Column(String(20), default="received")  # received, processing, responded, failed
